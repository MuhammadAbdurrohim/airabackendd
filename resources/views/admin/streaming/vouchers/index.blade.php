                        ...
                        <table class="table table-hover table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form action="{{ route('admin.streaming.vouchers.toggle-status', $voucher) }}" 
                                              method="POST" 
                                              class="d-inline">
                                            @csrf
                                            <button type="submit" 
                                                    class="btn btn-sm btn-{{ $voucher->active ? 'warning' : 'success' }}"
                                                    onclick="return confirm('Yakin ingin {{ $voucher->active ? 'menonaktifkan' : 'mengaktifkan' }} voucher ini?')">
                                                <i class="fas fa-{{ $voucher->active ? 'times' : 'check' }}"></i>
                                                {{ $voucher->active ? 'Nonaktifkan' : 'Aktifkan' }}
                                            </button>
                                        </form>
                                        
                                        <a href="{{ route('admin.streaming.vouchers.edit', $voucher) }}" 
                                           class="btn btn-sm btn-info">
                                            <i class="fas fa-edit"></i> Edit
                                        </a>
                                        
                                        <form action="{{ route('admin.streaming.vouchers.destroy', $voucher) }}" 
                                              method="POST" 
                                              class="d-inline">
                                            @csrf
                                            @method('DELETE')
